...
Use one of these commands to start a worker. Each worker owns one claimed task at a time; several workers may run at once.
...
## Run the adoption stage for one repository

```text
Execute ArcForges adoption task <ADOPT-ID> for <repository>. Read C:\MyFile\Projects\Plan-B\arcforges-implementation.md and C:\MyFile\Projects\ArcForges-Design-B\docs\planning\delivery\adoption.md completely. Classify every delivery task owned by the repository as inherited, inherited with adjustment, gap or conflicting under ADP-01 to ADP-06, using only reviewed evidence; record the report and inherited-task ledger records in the Plan ledger; raise conflicts under D-001; propose planning changes for adjustments that do not fit existing tasks. Do not execute implementation tasks during adoption.
```
